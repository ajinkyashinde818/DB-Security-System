...
public class AdminOP {
    ...
    public ArrayList getAdminCredentials(){
        ...
        try{
            ...
            if (rs.next()) {
            admindata.add(rs.getString(1)); // username
            admindata.add(rs.getString(3)); // password (or any other column)
            admindata.add(rs.getString(2)); // email (or any other column)
        } else {
   ...
   public boolean setAdminCredentials(String username, String password, String emailid) {
    ...
    try {
        ...
        // Always close resources outside the if block
        st.close();
        db.con.close();
